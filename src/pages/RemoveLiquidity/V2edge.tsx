...
import { CurrencyAmount, Percent } from "@panaromafinance/panaromaswap_sdkcore";
import { NonfungiblePositionManager } from "@panaromafinance/panaromaswap_v2edgesdk";
...
import { useV2edgeNFTPositionManagerContract } from "hooks/useContract";
import useDebouncedChangeHandler from "hooks/useDebouncedChangeHandler";
import useTheme from "hooks/useTheme";
import useTransactionDeadline from "hooks/useTransactionDeadline";
import { useV2edgePositionFromTokenId } from "hooks/useV2edgePositions";
import useNativeCurrency from "lib/hooks/useNativeCurrency";
import { useCallback, useMemo, useState } from "react";
import { Navigate, useLocation, useParams } from "react-router-dom";
import { Text } from "rebass";
import {
  useBurnV2edgeActionHandlers,
  useBurnV2edgeState,
  useDerivedV2edgeBurnInfo
} from "state/burn/v2edge/hooks";
...
const DEFAULT_REMOVE_V2edge_LIQUIDITY_SLIPPAGE_TOLERANCE = new Percent(5, 100);

// redirect invalid tokenIds
export default function RemoveLiquidityV2edge() {
  ...
  if (parsedTokenId === null || parsedTokenId.eq(0)) {
    return <Navigate to={{ ...location, pathname: "/pool/v1" }} replace />;
  }

  return <Remove tokenId={parsedTokenId} />;
}
function Remove({ tokenId }: { tokenId: BigNumber }) {
  const { position } = useV2edgePositionFromTokenId(tokenId);
  ...
  const {
    ...
    outOfRange,
    error
  } = useDerivedV2edgeBurnInfo(position, receiveWETH);
  const { onPercentSelect } = useBurnV2edgeActionHandlers();
  ...
  const allowedSlippage = useUserSlippageToleranceWithDefault(
    DEFAULT_REMOVE_V2edge_LIQUIDITY_SLIPPAGE_TOLERANCE
  ); // custom from users
    ...
    <AutoColumn>
      ...
      <AppBody>
        <AddRemoveTabs
          ...
          defaultSlippage={DEFAULT_REMOVE_V2edge_LIQUIDITY_SLIPPAGE_TOLERANCE}
        />
        <Wrapper>
          {position ? (
            <AutoColumn gap="lg">
              <RowBetween>
                <RowFixed>
                  ...
                  <ThemedText.DeprecatedLabel
                    ml="10px"
                    fontSize="20px">{`${feeValue0?.currency?.symbol}/${feeValue1?.currency?.symbol}`}</ThemedText.DeprecatedLabel>
                </RowFixed>
