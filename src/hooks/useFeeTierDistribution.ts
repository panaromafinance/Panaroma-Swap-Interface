...
export function useFeeTierDistribution(
  ...
  const { isFetching, isLoading, isError, distributions } = usePoolTVL(
    currencyA?.wrapped,
    currencyB?.wrapped
  );
  ...
  return useMemo(() => {
    if (isLoading || isFetching || isError || !distributions) {
      return {
...
function usePoolTVL(token0: Token | undefined, token1: Token | undefined) {
  ...
  const { isLoading, isFetching, isError, data } = useFeeTierDistributionQuery(
    token0 && token1
      ? {
  ...
  const { asToken0, asToken1, _meta } =
    (data as FeeTierDistributionQuery) ?? {};
  ...
  }, [_meta, asToken0, asToken1, isLoading, isError, isFetching, latestBlock]);
}
