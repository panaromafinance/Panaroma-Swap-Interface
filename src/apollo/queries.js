...
// const FACTORY_ADDRESS = '0xf8eD105de5D784753596E28401cC7a3a1B08C047';
//'0x7EFB2E0cDb32CF064d5a075cBD44B8B8238C90a6'

const BUNDLE_ID = '1'

export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "panaswap/panaswap-v2") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const V1_DATA_QUERY = gql`
  query panaswap($date: Int!, $date2: Int!) {
    current: panaswap(id: "1") {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
    oneDay: panaswapHistoricalDatas(where: { timestamp_lt: $date }, first: 1, orderBy: timestamp, orderDirection: desc) {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
    twoDay: panaswapHistoricalDatas(
...
export const POSITIONS_BY_BLOCK = (account, blocks) => {
  // console.log("1111 POSITIONS_BY_BLOCK", POSITIONS_BY_BLOCK);
  let queryString = 'query blocks {'
  queryString += blocks.map(
    (block) => `
      t${block.timestamp}:liquidityPositions(where: {user: "${account}"}) { 
...
export const HOURLY_PAIR_RATES = (pairAddress, blocks) => {
  ...
  queryString += blocks.map(
    (block) => `
      t${block.timestamp}: pair(id:"${pairAddress}") { 
        token0Price
        token1Price
      }
    `
...
export const ETH_PRICE = (block) => {
  ...
  `
  // console.log("11111 block", queryString);
...
export const GLOBAL_CHART = gql`
  query panaromaswapDayDatas($startTime: Int!, $skip: Int!) {
    panaromaswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query panaromaswapFactories {
    panaromaswapFactories(
...
export const PAIRS_HISTORICAL_BULK = (block, pairs) => {
  ...
  let queryString = `
  query pairs {
    pairs(first: 200, where: {id_in: ${pairsString}}, orderBy: trackedReserveETH, orderDirection: desc) {
      id
      reserveUSD
      trackedReserveETH
      volumeUSD
