...
const InfoCard = styled.button<{ isActive?: boolean }>`
  background-color: ${({ theme, isActive }) =>
    isActive ? theme.deprecated_bg3 : theme.deprecated_bg2};
  padding: 1rem;
  outline: none;
  border: 1px solid;
  border-radius: 12px;
  ...
  border-color: ${({ theme, isActive }) =>
    isActive ? "transparent" : theme.deprecated_bg3};
`;
...
const OptionCardClickable = styled(OptionCard as any)<{ disabled?: boolean }>`
  margin-top: 0;
  &:hover {
    cursor: ${({ disabled }) => (disabled ? "not-allowed" : "pointer")};
    border: ${({ disabled, theme }) =>
    disabled ? `1px solid ${theme.deprecated_primary1}` : ``};
  }
