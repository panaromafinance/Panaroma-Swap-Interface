...
import { FeeAmount, TICK_SPACINGS } from '@panaromafinance/panaromaswap_v2edgesdk'
import { TickData, TickProcessed } from 'hooks/usePoolTickData'
import JSBI from 'jsbi'

import computeSurroundingTicks from './computeSurroundingTicks'

const getV2edgeTick = (tick: number, liquidityNet: number): TickData => ({
  tick,
...
describe('#computeSurroundingTicks', () => {
  it('correctly compute active liquidity', () => {
    ...
    const sortedTickData: TickData[] = [
      getV2edgeTick(activeTickProcessed.tick - 4 * spacing, 10),
      getV2edgeTick(activeTickProcessed.tick - 2 * spacing, 20),
      getV2edgeTick(activeTickProcessed.tick - 1 * spacing, 30),
      getV2edgeTick(activeTickProcessed.tick * spacing, 100),
      getV2edgeTick(activeTickProcessed.tick + 1 * spacing, 40),
      getV2edgeTick(activeTickProcessed.tick + 2 * spacing, 20),
      getV2edgeTick(activeTickProcessed.tick + 5 * spacing, 20),
    ]
