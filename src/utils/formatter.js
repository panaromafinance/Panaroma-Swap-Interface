...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.panaroma.finance/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://swap.panaroma.finance/#/swap?inputCurrency=${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
      }&outputCurrency=${token1Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://swap.panaroma.finance/#/PANA/ETH/${token0Address}`
}

export function getPanaswapAppLink(linkVariable) {
  let basePanaswapUrl = 'https://swap.panaroma.finance/#/pana'
  if (!linkVariable) {
    return basePanaswapUrl
  }

  return `${basePanaswapUrl}/ETH/${linkVariable}`
}
...
export async function getBlocksFromTimestamps(timestamps, skipCount = 500) {
  ...
  // console.log("11111 fetchedData", fetchedData);
