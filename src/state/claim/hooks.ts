...
import { abi as MERKLE_DISTRIBUTOR_ABI } from "@panaromafinance/panaromaswap_merkledistributor/build/MerkleDistributor.json";
...
function fetchClaimMapping(): Promise<ClaimAddressMapping> {
  return (
    ...
    (FETCH_CLAIM_MAPPING_PROMISE = fetch(
      `https://raw.githubusercontent.com/Panaromaswap/mrkl-drop-data-chunks/final/chunks/mapping.json`
    )
      .then((res) => res.json())
      .catch((error) => {
...
function fetchClaimFile(
  ...
  return (
    ...
    (FETCH_CLAIM_FILE_PROMISES[key] = fetch(
      `https://raw.githubusercontent.com/Panaromaswap/mrkl-drop-data-chunks/final/chunks/${key}.json`
    )
      .then((res) => res.json())
      .catch((error) => {
        console.error(
...
// check if user is in blob and has not yet claimed PANA
...
export function useUserUnclaimedAmount(
  ...
  if (!canClaim || !userClaimData) {
    return CurrencyAmount.fromRawAmount(PANA, JSBI.BigInt(0));
  }
  return CurrencyAmount.fromRawAmount(PANA, JSBI.BigInt(userClaimData.amount));
}
        ...
        return distributorContract
          ...
          .then((response: TransactionResponse) => {
            addTransaction(response, {
              ...
              recipient: account,
              panaAmountRaw: unclaimedAmount?.quotient.toString()
            });
