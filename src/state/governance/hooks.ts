...
import { abi as GOVERNANCE_ABI } from "panaromaswap_governance/build/GovernorAlpha.json";
import { abi as UNI_ABI } from "panaromaswap_governance/build/Bang.json";
...
import { PANAROMASWAP_GRANTS_PROPOSAL_DESCRIPTION } from "constants/proposals/panaromaswap_grants_proposal_description";
...
import {
  ...
  ONE_BIP_START_BLOCK,
  POLYGON_START_BLOCK,
  PANAROMASWAP_GRANTS_START_BLOCK
...
export function usePanaromaContract() {
  const { chainId } = useWeb3React();
  const uniAddress = useMemo(
    () => (chainId ? PANA[chainId]?.address : undefined),
    [chainId]
  );
...
export function useAllProposalData(): {
  ...
  const proposalCount2 = useProposalCount(gov1);
  ...
  const PANA = useMemo(() => (chainId ? PANA[chainId] : undefined), [chainId]);
  ...
  return useMemo(() => {
    ...
    return {
      data: proposalsCallData.map((proposal, i) => {
        ...
        if (startBlock === PANAROMASWAP_GRANTS_START_BLOCK) {
          description = PANAROMASWAP_GRANTS_PROPOSAL_DESCRIPTION;
        }
...
export function useQuorum(
  ...
  return CurrencyAmount.fromRawAmount(PANA, quorumVotes);
}

// get the users delegatee if it exists
export function useUserDelegatee(): string {
  const { account } = useWeb3React();
  const uniContract = usePanaromaContract();
...
export function useUserVotes(): {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      votes:
        PANA && result
          ? CurrencyAmount.fromRawAmount(PANA, result?.[0])
          : undefined
    };
...
export function useUserVotesAsOfBlock(
  ...
  return votes && PANA ? CurrencyAmount.fromRawAmount(PANA, votes) : undefined;
}
...
export function useDelegateCallback(): (
  ...
  return useCallback(
    (delegatee: string | undefined) => {
      ...
      if (!uniContract) throw new Error("No PANA Contract!");
...
export function useProposalThreshold(): CurrencyAmount<Token> | undefined {
  ...
  if (res?.result?.[0] && PANA) {
    return CurrencyAmount.fromRawAmount(PANA, res.result[0]);
  }
